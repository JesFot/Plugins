...
public class LoginPrompt extends StringPrompt
{
	...
	public Prompt acceptInput(ConversationContext context, String input)
	{
		...
		if(this.sls.login(this.player, input))
		{
			...
			GamingBlockPlug_1_11.getMyLogger().info(player.getName() + " succesfuly logged in with his password.");
		...
		else
		{
			...
			GamingBlockPlug_1_11.getMyLogger().info(player.getName() + " used wrong password ('" + input + "') for the "
			...
			if(tryes >= 5)
			{
				...
				this.sls.endLogin(player);
				GamingBlockPlug_1_11.getMe().getLogger().info(player.getName() + " was kick for 5 invalid passwords.");
				GamingBlockPlug_1_11.getMyLogger().info(player.getName() + " kicked for wrong passwords.");
				return Prompt.END_OF_CONVERSATION;
			}
			else
			{
